...
**See also:** [Oracles for Logic Bugs in Existing Code](/logic-bug-oracles) (the direct successor — answers the open question this essay left), [Deterministic Simulation Testing](/deterministic-simulation-testing) and [The Decision Stream](/decision-stream) (the first arc — antithesis → oracle → lever → Shannon → normalize → "collapse the valid-program space"), normalize's [`inferred-opinionation`](https://github.com/rhi-zone/normalize/blob/master/docs/design/inferred-opinionation.md) (the constructive-leverage destination), [Vision](/vision), [Ecosystem Design Principles](/decisions/throughlines).
