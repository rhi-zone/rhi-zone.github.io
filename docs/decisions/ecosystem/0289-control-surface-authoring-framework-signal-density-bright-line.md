...
- ~~C4: finish the `.claude/commands/*.md` -> `.claude/skills/<name>/SKILL.md` migration, or drop the self-exempting fence~~ — **DONE (2026-06-29).** Migration executed (hub + 32/37 receivers), `sync-skills.sh` rewritten atomic-convergent directory-only, CLAUDE.md fence removed; 5 dirty recipients dirty-deferred (converge on next clean sync). A latent `.githooks/pre-commit` re-stage bug surfaced and was fixed (template + 11 receivers), with an open ecosystem-wide sweep. See Resolution above.
