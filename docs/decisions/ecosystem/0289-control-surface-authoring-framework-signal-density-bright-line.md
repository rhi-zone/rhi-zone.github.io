...
## Status
Accepted — rewrite implemented 2026-06-29; propagation executed 2026-06-29 (39 pushed / 9 dirty-additive / 3 skipped / 3 residual owner-action). C4 resolved 2026-06-29 (the last open fork).
...
**Propagation — EXECUTED 2026-06-29 (no longer held).** The 54-repo harness propagation was run. Hub (github-io) pushed to origin/master: `2c90224` (CLAUDE.md rewrite) and `33c1620` (propagation-resilience fix — per-repo failure isolation + classified end-of-run summary, replacing the old whole-batch abort-on-first-push-failure). Rollout: 39 clean repos committed + pushed (the dangling `throughlines.md` reference / old propagated region is now replaced and live in the clean set); 9 dirty repos got harness-only additive commits (never pushed, per policy); 3 skipped/already-current (incl. one expected-skip recipient — converged to a no-op; its push fails by design under separate production credentials). 3 residual FAILURES, all owner-action and left safe: `rhizone/fractal` (no remote; committed local-only `6088283`), `rhizone/normalize` (`normalize rules run` errored `missing-summary` — `tooling/claude-hooks/` lacked a `SUMMARY.md`; now mitigated, see below), `pterror/software-taxonomy` (0-byte/corrupt `flake.lock` blocks `nix develop` → bun validator). **Tooling follow-ups — closed out 2026-06-29:** (1) *Commit receivers inside each receiver's own dev env* — **VERIFIED ALREADY SATISFIED:** `propagate-harness-all.sh` already wraps the receiver commit (and `normalize init`) in `direnv exec .` from inside the receiver dir, so the receiver's own env is active; the earlier "cargo: command not found" symptom was repo-specific (normalize's missing-`SUMMARY` rule, software-taxonomy's empty `flake.lock`), not an env-wrapping gap (nested `direnv exec` swaps rather than stacks envs — retained as a caveat). (2) *Ship a `SUMMARY.md` in the `tooling/claude-hooks/` payload* — **DONE (commit `d7b5846`):** added `tooling/claude-hooks/SUMMARY.md`, wired into the propagator's `HOOK_FILES`, so it propagates; this also unblocks normalize's `missing-summary` rule on the next clean re-propagation. (3) *Propagate the `.githooks/` private-names guards ecosystem-wide* — **WON'T DO (deprioritized):** both guards (`pre-commit` content-scan + a `commit-msg` message-scan added 2026-06-29 after a denylisted name leaked into a commit message) stay github-io-local. Most receiver repos don't reference other projects, so the private-names denylist guard earns nothing there; the guard belongs where private cross-project names actually appear — github-io. A deliberate decision, not an open item.

**C4 — RESOLVED 2026-06-29 (was the last open fork).** The actual `.claude/commands/*.md` → `.claude/skills/<name>/SKILL.md` migration was executed: 8 canonical hub skills moved to directory-per-skill `SKILL.md`, `sync-skills.sh` rewritten to atomic-convergent directory-only sourcing, the CLAUDE.md C4 fence removed. all 37 recipients now on the skills/ layout; the **5 previously-dirty recipients (aeriea, defocus, normalize, rainbow, solarium)** were converged 2026-06-29 via a SCOPED additive commit (only `.claude/skills/` additions + managed `.claude/commands/` removals staged; each owner's WIP left untouched and uncommitted; NOT pushed per dirty-repo policy — local-only for the owner to push alongside their WIP; normalize's landed on its checked-out feature branch and passed that repo's own pre-commit rules). One repo (rhizone/server-less) left for the owner (a stray migration commit on a WIP feature branch).

**Pre-commit re-stage bug — FIXED 2026-06-29 (latent, surfaced by C4).** Scaffolded `.githooks/pre-commit` hooks re-staged after formatting via `git diff --cached --name-only | xargs git add`, re-adding DELETED paths and fatally aborting any commit containing a deletion (this blocked the commands-removal commits). Fixed the scaffold template (`scaffolding/.githooks/pre-commit`, commit `cdc4500`) with `--diff-filter=ACMR` and the 11 live receiver hooks carrying it. **Ecosystem-wide sweep — DONE 2026-06-29:** scanned 59 repos across all 5 org roots; ZERO new buggy repos beyond the already-fixed 11 + scaffold template. Every repo with a re-stage step is guarded (`--diff-filter=ACMR`/`ACM`/`d` or `git add -u`). The earlier fix covered the full exposure.
...
- ~~C4: finish the `.claude/commands/*.md` -> `.claude/skills/<name>/SKILL.md` migration, or drop the self-exempting fence~~ — **DONE (2026-06-29).** Migration executed (hub + all 37 receivers), `sync-skills.sh` rewritten atomic-convergent directory-only, CLAUDE.md fence removed; the 5 previously-dirty recipients converged via a scoped additive commit (owner WIP untouched, unpushed — owner to push). A latent `.githooks/pre-commit` re-stage bug surfaced and was fixed (template + 11 receivers); the ecosystem-wide sweep is now **DONE** (59 repos scanned, zero new buggy repos). See Resolution above.
- ~~Propagation (54 repos) + github-io push~~ — **DONE 2026-06-29.** Executed: hub pushed (`2c90224`, `33c1620`); 39 clean pushed / 9 dirty-additive / 3 skipped / 3 residual owner-action (`fractal` no-remote, `normalize` missing-summary — now mitigated by the shipped `tooling/claude-hooks/SUMMARY.md` (`d7b5846`) for the next clean re-propagation, `software-taxonomy` corrupt flake.lock). The previously-pending drift (SendMessage fix `fb1f9c1`, plan-mode stand-down `8d6b2d9`, orchestrator-rules stance rewrite `87cde38`) shipped with this propagation. The tooling follow-ups are now closed out (dev-env verified already satisfied; SUMMARY.md shipped; `.githooks/` ecosystem-propagation deprioritized — won't-do). See Resolution for the full outcome.
