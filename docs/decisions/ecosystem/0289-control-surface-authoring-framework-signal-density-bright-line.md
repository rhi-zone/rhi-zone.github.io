...
**C4 — RESOLVED 2026-06-29 (was the last open fork).** The actual `.claude/commands/*.md` → `.claude/skills/<name>/SKILL.md` migration was executed: 8 canonical hub skills moved to directory-per-skill `SKILL.md`, `sync-skills.sh` rewritten to atomic-convergent directory-only sourcing, the CLAUDE.md C4 fence removed. 32/37 recipients on the skills/ layout; **5 dirty recipients (aeriea, defocus, normalize, rainbow, solarium) dirty-deferred** — they converge on the next clean `sync-skills.sh` run. One repo (rhizone/server-less) left for the owner (a stray migration commit on a WIP feature branch).
...
- ~~C4: finish the `.claude/commands/*.md` -> `.claude/skills/<name>/SKILL.md` migration, or drop the self-exempting fence~~ — **DONE (2026-06-29).** Migration executed (hub + 32/37 receivers), `sync-skills.sh` rewritten atomic-convergent directory-only, CLAUDE.md fence removed; 5 dirty recipients dirty-deferred (converge on next clean sync). A latent `.githooks/pre-commit` re-stage bug surfaced and was fixed (template + 11 receivers); the ecosystem-wide sweep is now **DONE** (59 repos scanned, zero new buggy repos). See Resolution above.
