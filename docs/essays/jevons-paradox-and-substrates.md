...
## Crescent's anti-Jevons strategy

"Cover the entire surface area of software" sounds intractable stated cold — software's
surface looks unbounded. But it isn't, once decomposed: for any given complexity level,
there are finitely many useful libraries. Each one is a bounded, solvable problem — an HTTP
client, a DNS resolver, a storage layer are each finite pieces of work, not an open-ended
commitment. And the pieces compose: a library built on top of two already-solved libraries
inherits their coverage for free, so each addition reduces the marginal entropy of
everything built on top of it. The surface shrinks faster than linearly as the bottom fills
in, because later libraries lean on earlier ones instead of re-solving them.

The strategy for filling that surface is dogfooding plus a roadmap: build real, useful
example apps in the most commonly needed categories, and let building them drive what gets
built underneath. This serves double duty:

1. It forces libraries into existence — every gap hit while building a real app (no HTTP
   client, no session storage, no whatever) becomes a library, because the app can't ship
   without it. The roadmap is generated by contact with real requirements, not guessed at
   in advance.
2. The apps themselves preempt the AI-generation loop this essay opened with. Someone who
   needs "that kind of app" forks the example instead of asking an agent to generate the
   whole thing from scratch — they write the correction terms specific to their case, on
   top of a working substrate, rather than regenerating the ceremony every layer down.

This is anti-Jevons at every layer, not just one. The libraries reduce encoding overhead
for anyone building on crescent — that's the substrate lever already described above. The
example apps reduce it a layer further, by covering common use cases end-to-end so the
starting point for a new app is a working one, not an empty one. Each layer cascades into
the next: better libraries make better example apps cheaper to build, and the example apps
in turn surface which libraries are still missing.

Notably, this requires no abstract taxonomy of "software's surface area" drawn up in
advance. The roadmap comes from building real things and tracking what's missing while
building them — the coverage hierarchy emerges from use, not from mapping the space
theoretically first.
...
## Security and commodity intelligence

Cheap code generation has a secondary consequence: everything that isn't already built gets a
bespoke software layer. Each new layer is a new attack surface. But substrates that cover
their domain's breadth upfront resist this exponentially — not because security is harder,
but because the surfaces don't exist to attack. Crescent's vendoring-first model, with no
transitive dependencies and no supply chain, is structurally resistant to the attack-surface
explosion that centralized package registries amplify: you can't poison what you own, and
you can't exploit a dependency chain that doesn't exist. Reducing encoding overhead also reduces
the *volume* of generated code needing review or securing — fewer subagents spawned means
fewer surfaces to defend.

The second pressure is commodity intelligence. As models shrink and run on consumer hardware,
"code generation" stops being a specialized capability and becomes something anyone with a
laptop can do. This makes the Jevons problem worse, not better — more participants running
agents means more unrestricted demand, and outrunning that demand with supply buildout
becomes even less tenable. In this world, the substrate lever shifts from "optimization" to
"the only compression point left." A substrate that compresses the recursion tree is no longer
a nice-to-have efficiency; it's the mechanism between bounded and unbounded demand.
