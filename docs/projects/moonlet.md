...
Modular plugin architecture with 8 working integrations (fs, libm, normalize, packages, sessions, tools, llm, embed). Feature-complete for core mission (multi-provider LLM client + Lua runtime). One architectural task remaining: abstract LLM APIs to reduce rig coupling.
:::
