...
- [ ] **Why software is shaped wrong** — exoplace/github-io session (Jan 29, session 444dae7b). VERY rich — 7 sub-topics: objects-not-documents, bridge problems, incumbents aren't competent, two tracks of programming (lost word "hacker"), state-not-architecture, 8-item forcing function, dopamine arbitrage. Could be 2-3 essays. "Objects not documents" and "two tracks" are strongest standalone candidates. Grep: `notes`, `freeform`, `spreadsheet`, `paper`, `objects`, `documents`, `hacker`
- [ ] **Can programs write programs?** — prose sessions (Dec 16-17) + moss session. Fractal code structure, recursive abstraction ("programming as recursively less detailed views"), type-directed synthesis, reverse parsing, Merkle trees for codebase hierarchies. Possibly rhi docs rather than pteraworld essay. Rich material extracted from history.jsonl. Grep: `fractal`, `synthesis`, `recursive`, `Merkle`
- [ ] **What counts as vibe coding?** — normalize session (Jan 21, session 4a92450f). Hallucination reframed as feedback-loop problem, specification gaming / reward hacking (AI optimizing for passing not correctness), epistemic honesty at scale. Heavy overlap with existing essays — main NEW insight is spec gaming. Could fold into existing rather than standalone. Grep: `vibe coding`, `hallucinate`, `reward hacking`, `specification gaming`
...
- [ ] **Why nothing feels special / why did things stop being personal?** — github-io session (Feb 25, session b08e9487). Existing essay `why-is-nothing-special-anymore.md` covers convergence. Session extends with: personal web economics, GeoCities grief, Tumblr paradox ("failure preserved it"), platform convergence, "nostalgia for something you never had." Could update existing essay or write companion. Grep: `special`, `safe`, `personal`, `GeoCities`, `Tumblr`
...
- [ ] **Where does meaning live? / Why doesn't the engine know what anything means?** — aspect session (Jan 29, project path: `aspect`). Broader than aspect: the principle that abstractions shouldn't encode specifics. normalize doesn't know what "a function" means, existence doesn't hardcode what "sadness" is, wick doesn't know its evaluation domain. "An abstraction that encodes specifics is just a shortcut pretending to be a concept." Connects to reinvention (calcified assumptions), emergence (meaning arrives through use), architect-vs-inhabit. Grep: `semantic-free`, `world packs`, `substrate`
